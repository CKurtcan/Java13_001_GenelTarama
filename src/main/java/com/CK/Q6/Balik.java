package com.CK.Q6;

public class Balik extends SuCanlilari{

    public Balik() {
        super();
    }

    public Balik(String tur, String ad, String ureme, String hareket, int yas) {
        super(tur, ad, ureme, hareket, yas);
    }

    @Override
    public String toString() {
        return super.toString();
    }

    @Override
    public String getTur() {
        return super.getTur();
    }

    @Override
    public void setTur(String tur) {
        super.setTur(tur);
    }

    @Override
    public String getAd() {
        return super.getAd();
    }

    @Override
    public void setAd(String ad) {
        super.setAd(ad);
    }

    @Override
    public String getUreme() {
        return super.getUreme();
    }

    @Override
    public void setUreme(String ureme) {
        super.setUreme(ureme);
    }

    @Override
    public String getHareket() {
        return super.getHareket();
    }

    @Override
    public void setHareket(String hareket) {
        super.setHareket(hareket);
    }

    @Override
    public int getYas() {
        return super.getYas();
    }

    @Override
    public void setYas(int yas) {
        super.setYas(yas);
    }

    @Override
    public int getYuzgecSayisi() {
        return super.getYuzgecSayisi();
    }

    @Override
    public void setYuzgecSayisi(int yuzgecSayisi) {
        super.setYuzgecSayisi(yuzgecSayisi);
    }

    @Override
    public String getYuzgecTipi() {
        return super.getYuzgecTipi();
    }

    @Override
    public void setYuzgecTipi(String yuzgecTipi) {
        super.setYuzgecTipi(yuzgecTipi);
    }
}
